...
def edit_task(task_list, user_tasks, selected_task, task_index):
    ...
    """
    # Find the index of the selected_task in the task_list
    index_in_task_list = next((i for i, task in enumerate(task_list) if task == selected_task), None)
    
    if index_in_task_list is not None:
        ...
        if choice == "1":
            ...
            task_list[index_in_task_list]['username'] = new_username
        elif choice == "2":
            while True:
                try:
                    new_due_date = input("Enter new due date (YYYY-MM-DD): ")
                    task_list[index_in_task_list]['due_date'] = datetime.strptime(
        ...
        if user_input == '-1':
            ...
            if 0 <= task_index < len(user_tasks):
                ...
                if selected_task['completed']:
                    ...
                    edit_choice = input(
                        '''Do you want to mark the task as complete (enter 'complete') or 
                        edit the task (enter 'edit')?\n''').lower()
...
def generate_reports(username_password, task_list):
    ...
    with open("task_overview.txt", "w") as report_file:
        ...
        if total_tasks != 0:
            report_file.write(f"Percentage of tasks that are incomplete: {incomplete_tasks /
            total_tasks * 100:.2f}%\n")
            report_file.write(f"Percentage of tasks that are overdue: {overdue_tasks / 
            total_tasks * 100:.2f}%\n")
        else: 
    ...
    with open("user_overview.txt", "w") as report_file:
        ...
        for username, password in username_password.items():
            ...
            completed_user_tasks = sum(1 for task in task_list if task['username'] ==
            username and task['completed'])
            incomplete_user_tasks = user_tasks - completed_user_tasks
            overdue_user_tasks = sum(1 for task in task_list if task['username'] ==
            username and not task['completed'] and task['due_date'] < datetime.now())
            report_file.write(f"\n{username}:\n")
            report_file.write(f"Total number of tasks assigned: {user_tasks}\n")
            if user_tasks != 0:
                report_file.write(f"Percentage of total tasks assigned: {user_tasks / 
                total_tasks * 100:.2f}%\n")
                report_file.write(f"Percentage of completed tasks: {completed_user_tasks / 
                user_tasks * 100:.2f}%\n")
                report_file.write(f"Percentage of incomplete tasks: {incomplete_user_tasks / 
                user_tasks * 100:.2f}%\n")
                report_file.write(f"Percentage of overdue tasks: {overdue_user_tasks / 
                user_tasks * 100:.2f}%\n")
            else:
